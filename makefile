...
# Repeat randomized weighted runs on the proposed SYNTHETIC input (50 reps)
run_weighted_randomized: $(WEIGHTED_EXEC)
	$(PYTHON) scripts/run_weighted_randomized_average.py tests_random/SYNTHETIC.txt --reps 50 --seed 12345 --max-rows-per-mode 0 --output-summary SYNTHETIC_average_summary_50.csv --output-raw SYNTHETIC_average_runs_50.csv
